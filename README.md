...
分别微调 **SmolVLA-450M**、**Octo** 与 **π0.5 (pi0.5)** 三个 VLA 模型，
...
```bash
# 在服务器上（双卡 DDP，每卡 bs=4，effective batch = 8）
bash train/pi05/run_pi05_lora_r16.sh

# 实例只有一块卡时，去掉 torchrun 并把 --batch_size 改成 8，
# 以保持 effective batch = 8 不变（见脚本头部注释）
```
...
| batch / lr / steps | effective **8** / 1.5e-5 / 20000（训练在 18625 步主动停止，末次存档 **step 018000**） |

> **真机用的是第 4.5 epoch 的模型**（step 018000）。各模型的 checkpoint 与 epoch 对照见
> [真机测试结果](#真机测试结果) 开头。
...
这是现场录像的直观对比，不是量化指标 —— 三个模型的 `--exec-steps` 并不相同（π0.5 是 20，SmolVLA 是 15，Octo 是 4），
每轮重规划的频率也就不一样，因此**这不构成对骨干架构的公平比较**，只说明 π0.5 在本任务上可用且动作质量最好。
...
##### 这个失败有多确定？用两个量把「语言没被读」量出来

π0.5 的推理**不固定随机种子**，同一帧同一指令连跑结果都不同，所以必须先量出「纯随机的抖动有多大」：
...
| **语言差 ÷ 噪声地板** | **0.96** | **0.47** | **0.49** | **0.43** | **0.67** | **0.51** |

判读的钥匙是一个统计预言：**若语言完全不起作用**，「换指令」带来的差异应该恰好是「重新采样」的一半
—— 因为前者用 4 次均值之差（std = σ/2），后者用单次之差（std = σ√2），比值恰为 **0.50**。
实测 6 个关节里 4 个正落在 0.50 附近，**换指令带来的动作变化与一个字不改纯重新采样完全一样大**。

**结论：语言变化的敏感度极低，模型忽略了语言指令中的目标物体。** 它实际上是个纯视觉的「选物器」——
在同一批帧上，模型确实会朝**画面上那个目标**转（与目标方位 r = +0.75），
但它靠的是**长相**，不是指令。根因在数据：桌上只有一个树，指令说「抓树」与视觉线索**永远冗余**，
微调没有梯度去保住语言敏感度。这也解释了为什么它和 Octo 犯的是同一个病。

> 顺带排除的假设：**不是输入分辨率**。把图降到 112×84（像素少 30 倍）再喂给同一个 serve，
> 选目标的表现纹丝不动，到 80×60 才松动 —— 模型用的特征本来就很粗，提到 512 不会有收益。
...
|---|---|---|---|---|---|---|---|---|---|
| **SmolVLA-450M** | 450 M | 全量微调 | 512×512 | ≈ 300 ms<br>（A2/A3 另加 YOLO ≈ 40 ms） | ≈ 140 ms | **≈ 480 ms** | 15 步 | 13 ~ 15 | ≈ 6 ~ 7 s |
| **Octo Base** | 93 M | LoRA rank 8<br>（仅 attention Q/V） | 256×256（primary）<br>128×128（wrist） | ≈ 130 ms | ≈ 50 ms | **≈ 180 ms** | 4 步 | 55 ~ 60 | ≈ 10 ~ 11 s |
| **π0.5** | 3.6 B | LoRA rank 16 | 224×224 | ≈ 450 ms | ≈ 100 ms | **≈ 550 ms** | 20 步 | 14 ~ 16 | ≈ 14 s |

- **π0.5 单轮规划约 550 ms，其中服务器云端推理约 450 ms，其余约 100 ms 是网络往返。**
  整体时长取自演示录像的实测长度（`dual_PI05_tree_*.mp4`，14.2 ~ 14.5 s）。
- 三个模型的输入分辨率**依次递减**（512 → 256 → 224），但单轮耗时并不同向变化：
  π0.5 骨干比 SmolVLA-450M 大一个数量级（3.6 B vs 450 M），分辨率更低却更慢。
- 单轮快慢与整体快慢是两回事：Octo 单轮最快（≈180 ms），但每轮只推进 **4 步**，
  完成同一任务要 55 ~ 60 轮，整体反而最慢。**决定整体耗时的是「每轮推进多少步」，不是单轮多快。**
...
│   │   └── probes/                    语言敏感度探针与对比实验
│   └── pi05/
│       ├── run_pi05_lora_r16.sh        LoRA(r16) 微调
│       └── probes/
...
| 其他组件 | 说明 |
|---|---|
| SmolVLA 基座 | `lerobot/smolvla_base`（450M）；A1/A2/A3 为**全量微调**，非 LoRA |
