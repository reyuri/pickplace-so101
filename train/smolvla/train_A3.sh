#!/usr/bin/env bash
# A3 = 红框 + 框感知指令（the {toy} in red box）
#
#   bash train_A3.sh                       # WORK 默认 /root/autodl-tmp
#   WORK=/data/toy bash train_A3.sh        # 换工作根目录
#   nohup bash train_A3.sh > train_A3.log 2>&1 &
#
# 目录约定（都在 WORK 下）：
#   lerobot/                    lerobot 源码 checkout（含本项目对 eval 切分的补丁）
#   smolvla_train_launcher.py   训练启动器（train/smolvla/ 下这份，拷到 WORK 或改下面路径）
#   lerobot_rey_toy_all_redbox_a3/   训练数据集（LeRobot v3.0）
#   outputs/train_A3/        输出：checkpoints/005000/pretrained_model 是本项目选中的权重
set -euo pipefail

WORK="${WORK:-/root/autodl-tmp}"
CONDA_ENV="${CONDA_ENV:-smolvla}"
DATASET="lerobot_rey_toy_all_redbox_a3"
...
exec python "$WORK/smolvla_train_launcher.py" \
  ...
  --output_dir="$WORK/outputs/train_A3" \
  --job_name=toy_all_A3 \
