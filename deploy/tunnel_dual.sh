#!/usr/bin/env bash
# 本地→服务器(SSH, 端口32077)建立 3 条推理隧道: 本地 6010/6011/6012 -> 服务器 6010/6011/6012。
# 依赖: ~/.ssh/config 已配 Host connect.bjb1.seetacloud.com(端口32077 + IdentityFile ~/.ssh/autodl_rey)。
# 用法(本地 Git Bash): bash tunnel_dual.sh {start|stop|status}
#   start 后, 本地脚本 drive_so101_dual.py 就可访问 http://127.0.0.1:6010/6011/6012。
#
...
start() {
  ...
  nohup ssh -o BatchMode=yes -o ExitOnForwardFailure=yes -N \
    ...
    "$REMOTE" > "$TUN_LOG" 2>&1 &
  echo "tunnel launcher pid=$!  (6010/6011/6012 -> server)"
  sleep 3
  status
}
